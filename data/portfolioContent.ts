...
export const projects = [
  ...
  {
    name: 'devfive-console',
    category: '실무 및 인턴',
    organization: '(주)데브파이브',
    period: '진행 중',
    role: '풀스택 개발',
    contribution:
      '공공조달 공고를 탐색·검토하는 사내 운영 콘솔을 작업하면서, 기존 사내 HMAC 인증 구현을 재사용하고 서비스 간 역할을 계층화해 콘솔이 내부 데이터 활용 지점으로 동작하도록 구성했습니다.',
    outcome:
      '반복적인 공고 탐색과 첨부문서 확인 과정을 줄이고, 기존 인증 흐름을 유지한 채 내부 서비스 확장에 맞는 운영 콘솔 계층을 마련했습니다.',
    summary:
      'DevFive 경영진과 실무자가 공공조달 공고를 빠르게 탐색하고 상세 정보와 첨부문서를 안정적으로 확인할 수 있도록 구축한 사내 운영 콘솔입니다.',
    techStack: ['Rust', 'Axum', 'SeaORM', 'PostgreSQL', 'S3', 'JWT', 'Cron'],
    highlights: [
      '나라장터 목록은 Cron 기반으로 주기 수집하되 상세 정보는 사용자가 공고를 열 때 조회하는 on-demand 방식으로 분리해, 불필요한 외부 API 호출과 초기 적재 비용을 줄였습니다.',
      '외부 공고 식별자를 기준으로 PostgreSQL 캐시를 조회하고 만료·누락된 데이터만 갱신하도록 구성해 동일 공고를 반복 확인할 때의 응답 지연과 외부 서비스 의존도를 낮췄습니다.',
      '원본 첨부파일 URL의 만료와 접근 제약을 피하기 위해 서버가 문서를 수집해 S3에 재호스팅하고, 공고 메타데이터와 저장 객체를 연결해 운영 화면에서 안정적으로 열람하도록 구현했습니다.',
      '사내 서비스에서 이미 사용 중이던 서비스 ID, 요청 시각, 본문 해시 기반 HMAC-SHA256 검증 방식을 그대로 재사용해 신규 콘솔도 기존 내부 인증 규칙 안에서 통신하도록 맞췄습니다.',
      '수집·상세 조회·문서 재호스팅 책임을 내부 서비스 계층으로 나누고, devfive-console은 그 결과를 탐색·검토하는 운영 계층으로 두어 서비스 확장 시 역할 경계가 흐려지지 않도록 정리했습니다.',
  ...
  },
  {
    ...
    role: '풀스택 개발',
    contribution:
      '정기결제와 라이선스 발급·활성화 흐름을 분리해, 과금 상태와 권한 부여를 추적 가능하게 만드는 포털의 풀스택 구조를 설계했습니다.',
    outcome:
      '결제 상태 전이와 라이선스 권한을 분리해 감사 가능한 과금 워크플로우와 운영 가능한 활성화 구조를 마련했습니다.',
    summary:
      '정기결제 상태와 라이선스 발급·활성화 과정을 분리해 과금과 권한을 안정적으로 운영하기 위한 서비스 포털입니다.',
    techStack: ['Next.js', 'Rust', 'PostgreSQL'],
    highlights: [
      '정기결제를 신청·승인·활성·연체·해지 상태로 관리하고 상태 변경 시점과 원인을 별도 이력으로 남겨, 현재 값만으로는 알 수 없는 과금 변경 과정을 추적하도록 설계했습니다.',
      '결제 성공을 즉시 권한으로 치환하지 않고 결제 처리와 라이선스 발급을 별도 도메인으로 분리해, 재처리나 오류가 발생해도 과금 기록과 사용자 권한이 서로 덮어쓰이지 않도록 구성했습니다.',
      '결제 흐름은 신청, 승인, 활성, 연체, 해지 등 가능한 상태와 전이 조건을 상태 머신처럼 정리하고, 허용되지 않은 전이가 발생하지 않는지 테스트하며 과금 워크플로우를 검증했습니다.',
      '라이선스 키의 발급, 사용자 연결, 기기 활성화와 비활성화 흐름을 분리하고 각 단계의 상태를 저장해 운영자가 권한 부여 과정을 확인할 수 있도록 했습니다.',
      'Next.js 포털은 사용자 결제·라이선스 상태를 표현하고 Rust API는 상태 전이 규칙을 검증하며 PostgreSQL은 변경 이력을 보존하도록 계층별 책임을 나눴습니다.',
    ],
  },
  {
    name: 'MYCMS',
    ...
    highlights: [
      ...
      '프로젝트 상세를 고정된 소개문 하나가 아니라 구현 주제별 섹션으로 누적하도록 구성해, 기술 선택과 문제 해결 기록을 필요에 따라 선별·재조합할 수 있게 했습니다.',
      'Evidence Graph에서 프로젝트 구현, 문서, 수상과 학습 기록을 노드로 연결하고 AI 생성 결과가 참조한 근거를 저장해 결과 문장에서 원본 기록까지 역추적하도록 구현했습니다.',
      '캘린더 마감일, 기회 탐색, 지원서, 증빙서류와 최근 작업을 하나의 대시보드에 연결해 기록 관리와 실제 지원 준비가 분리되지 않는 작업 흐름을 만들었습니다.',
      '검색·분류·핀 고정·최근 수정 정보를 공통 탐색 규칙으로 정리해 기록이 증가해도 필요한 근거를 다시 찾을 수 있도록 정보 접근 구조를 설계했습니다.',
    ],
  },
  {
    name: 'KB_NEWS_AI',
  ...
  },
  {
    ...
    role: '웹 풀스택 개발자',
    contribution:
      '공개 홈페이지와 관리자 시스템을 개발하면서 검색·좌표 모델링·컨테이너 배포 자동화까지 사용자 기능과 운영 환경을 함께 설계했습니다.',
    outcome:
      '검색과 관리자 운영 기능을 갖춘 홈페이지를 구축하고, CI/CD와 GitOps 기반 배포 흐름으로 반복 배포 가능한 운영 환경을 마련했습니다.',
    summary:
    ...
    highlights: [
      ...
      'GitHub Actions가 이미지를 생성하고 배포 선언을 갱신하면 Argo CD가 변경을 감지해 Kubernetes에 반영하는 GitOps 흐름을 구축했습니다.',
  ...
  },
  {
    ...
    highlights: [
      ...
      'react-joyride의 단계별 target을 실제 인터랙션 요소와 연결하고 화면 크기에 따라 안내 위치를 조정해, 첫 방문자가 별도 설명 없이 체험 흐름을 따라가도록 구현했습니다.',
      '색상 토큰과 컴포넌트 상태를 기준으로 라이트·다크 테마를 통합해 모드 전환 시 비교 화면과 튜토리얼의 대비가 깨지지 않도록 정리했습니다.',
      '입력, 변환 결과, 테마 전환과 튜토리얼 동작을 Vitest로 검증하고 테스트 커버리지 100%를 유지해 랜딩 페이지 변경의 회귀 위험을 낮췄습니다.',
    ],
  },
  {
  ...
  },
  {
    ...
    highlights: [
      ...
      '종목이 선택되지 않았거나 필수 식별자가 준비되지 않은 상태에서는 요청 키를 생성하지 않도록 구성해, 초기 렌더링과 화면 전환 과정의 잘못된 API 호출을 차단했습니다.',
      '주가·뉴스·AI 분석 결과의 로딩과 오류 상태를 독립적으로 관리해 한 데이터 소스의 지연이 전체 화면을 막지 않고 준비된 정보부터 표시되도록 구성했습니다.',
    ],
    links: [
      {
        label: '소스 코드',
